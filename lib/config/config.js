 ...
 * @property {string}                           configDirectoryPath     Path to the wpnv.json file.
 * @property {string}                           workDirectoryPath       Path to the work directory located in ~/wpnv.
...
/**
 * Reads, parses, and validates the given wpnv.json file into a wp-env config
 * object for internal use.
 *
 * @param {string} configPath Path to the wpnv.json file.
 *
...
module.exports = async function readConfig(configPath) {
  ...
  // The specified base configuration from wpnv.json or from the local
  // source type which was automatically detected.
  const baseConfig =
    (await readRawConfigFile('wpnv.json', configPath)) ||
    (await getDefaultBaseConfig(configPath))

  // Overriden wpnv.json on a per-user case.
  const overrideConfig =
    (await readRawConfigFile(
      'wpnv.override.json',
      configPath.replace(/\wpnv\.json$/, 'wpnv.override.json')
    )) || {}

  const detectedLocalConfig =
    Object.keys({ ...baseConfig, ...overrideConfig }).length > 0

  // Default configuration which is overridden by wpnv.json files.
  ...
  validateConfig(
    mergeWpServiceConfigs([defaultConfiguration, baseConfig, overrideConfig])
  )
  ...
  if (allPorts.size !== allEnvs.length) {
    throw new ValidationError(
      'Invalid wpnv.json: Each port value must be unique.'
    )
  }
  ...
  const mergeNestedObjs = (key) =>
    Object.assign(
      ...
      ...configs.map((config) => {
        ...
        throw new ValidationError(
          `Invalid wpnv.json: "${key}" must be an object.`
        )
      })
    )
...
/**
 * Detects basic config options to use if the wpnv.json config file does not
  ...
  const updateEnvUrl = (configKey) => {
    ;[
      ...
      try {
        ...
        throw new ValidationError(
          `Invalid wpnv.json: config.${configKey} must be a valid URL.`
        )
      }
    })
  }
 ...
 *
 * By default: '~/wpn/'. On Linux with snap packages: '~/wp-env/'. Can be
