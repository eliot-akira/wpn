 ...
 * @property {string}                           configDirectoryPath     Path to the wpn.json file.
 * @property {string}                           workDirectoryPath       Path to the work directory located in ~/wpn.
...
/**
 * Reads, parses, and validates the given wpn.json file into a wp-env config
 * object for internal use.
 *
 * @param {string} configPath Path to the wpn.json file.
 *
...
module.exports = async function readConfig(configPath) {
  ...
  // The specified base configuration from wpn.json or from the local
  // source type which was automatically detected.
  const baseConfig =
    (await readRawConfigFile('wpn.json', configPath)) ||
    (await getDefaultBaseConfig(configPath))

  // Overriden wpn.json on a per-user case.
  const overrideConfig =
    (await readRawConfigFile(
      'wpn.override.json',
      configPath.replace(/\wpn\.json$/, 'wpn.override.json')
    )) || {}

  const detectedLocalConfig =
    Object.keys({ ...baseConfig, ...overrideConfig }).length > 0

  // Default configuration which is overridden by wpn.json files.
  ...
  if (allPorts.size !== allEnvs.length) {
    throw new ValidationError(
      'Invalid wpn.json: Each port value must be unique.'
    )
  }
  ...
  const mergeNestedObjs = (key) =>
    Object.assign(
      ...
      ...configs.map((config) => {
        ...
        throw new ValidationError(
          `Invalid wpn.json: "${key}" must be an object.`
        )
      })
    )
...
/**
 * Detects basic config options to use if the wpn.json config file does not
  ...
  const updateEnvUrl = (configKey) => {
    ;[
      ...
      try {
        ...
        throw new ValidationError(
          `Invalid wpn.json: config.${configKey} must be a valid URL.`
        )
      }
    })
  }
