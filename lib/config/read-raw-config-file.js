...
module.exports = async function readRawConfigFile(name, configPath) {
  try {
    return withBackCompat(JSON.parse(await fs.readFile(configPath, 'utf8')))
  } catch (error) {
    if (error.code === 'ENOENT') {
      ...
      throw new ValidationError(`Could not read ${name}: ${error.message}`)
    }
  }
}

/**
 * Used to maintain back compatibility with older versions of the wpnv.json
 * file. Returns an object in the shape of the currently expected wpnv.json
 * version.
 *
  ...
  if (rawConfig.testsPort !== undefined) {
    rawConfig.env = {
      ...
      tests: {
        ...
        ...(rawConfig.env && rawConfig.env.tests ? rawConfig.env.tests : {}),
      },
    }
  }
