 ...
 * specified in wpnv.json, validates it, and converts it into the format used
 ...
 * @param {string} options.workDirectoryPath Path to the work directory located in ~/wpnv.
...
function parseSourceString(sourceString, { workDirectoryPath }) {
  ...
  try {
    ...
    if (supportedProtocols.includes(sshUrl.protocol)) {
      const pathElements = sshUrl.pathname.split('/').filter((e) => !!e)
      const basename = pathElements.slice(-1)[0].replace(/\.git/, '')
