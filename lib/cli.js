  ...
  (...args) => {
    ...
    return command(...args).then(
      (message) => {
        ...
        const text = (message || spinner.text || '').trim()
        if (text)
          spinner.succeed(
            text
...
module.exports = function cli() {
  ...
  try {
    ...
    console.error(chalk.red('Could not connect to Docker. Is it running?'))
    process.exit(1)
  }
  ...
  yargs.command(
    ...
    chalk`Starts WordPress for development on port {bold.underline ${terminalLink(
      '8888',
      'http://localhost:8888'
    )}} (override with WP_ENV_PORT). The current working directory must be a WordPress installation, a plugin, a theme, or contain a wpnv.json file. After first install, use the '--update' flag to download updates to mapped sources and to re-apply WordPress configuration options.`, //	),

    // and tests on port {bold.underline ${ terminalLink(
    // 	'8889',
    // 	'http://localhost:8889'
    // ) }} (override with WP_ENV_TESTS_PORT)
    (args) => {
      args.option('update', {
        type: 'boolean',
        describe: 'Download source updates and apply WordPress configuration.',
        default: false,
      })
      args.option('xdebug', {
        describe:
  ...
  yargs.command(
    'shell',
    // wpRed(
    'Opens an interactive shell (bash session) in the WordPress instance.', // ),
    () => {},
  ...
  yargs.command(
    'info',
    // wpRed(
    'Display information about running WordPress instances.', // ),
    () => {},
    withSpinner(env.info)
  )

  yargs.command(
    'export-db',
    // wpRed(
    'Export database to file db.sql.', // ),
    () => {},
    withSpinner(env.exportDB)
  )
  yargs.command(
    'import-db',
    // wpRed(
    'Import database from file db.sql.', // ),
    () => {},
    withSpinner(env.importDB)
  )

  yargs.command(
    'snapshot',
    'Makes a snapshot of the environment DB and files.',
    () => {},
    withSpinner(env.snapshot)
  )
  yargs.command(
    'restore',
    'Restores a snapshot of the environment DB and files.',
    () => {},
