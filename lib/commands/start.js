...
module.exports = async function start({ spinner, debug, update, xdebug }) {
  ...
  if (!config.detectedLocalConfig) {
    ...
    spinner.warn(
      `Warning: could not find a wpnv.json configuration file and could not determine if '${configDirectoryPath}' is a WordPress installation, a plugin, or a theme.`
    )
    spinner.start()
  }
...
async function checkForLegacyInstall(spinner) {
  ...
  await Promise.all(
    installs.map((install) =>
      fs
        .access(install)
        .catch(() => installs.splice(installs.indexOf(install), 1))
    )
  )
  if (!installs.length) {
    return
