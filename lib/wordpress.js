...
async function configureWordPress(environment, config, spinner) {
  ...
  const installCommand = `wp core install --url="${url}" --title="${config.name}" --admin_user=admin --admin_password=admin --admin_email=wordpress@example.com --skip-email`
  ...
  await dockerCompose.run(
    'cli', //environment === 'development' ? 'cli' : 'tests-cli',
  ...
  // 	'wordpress', // environment === 'development' ? 'wordpress' : 'tests-wordpress',
  // 	[
  // 		'sh',
  // 		'-c',
...
async function readWordPressVersion(coreSource, spinner, debug) {
  ...
  const versionMatch = versionFile.match(/\$wp_version = '([A-Za-z\-0-9.]+)'/)
  if (!versionMatch) {
    throw new Error(`Failed to find version in ${versionFilePath}`)
  }

  if (debug) {
    spinner.info(`Found WordPress ${versionMatch[1]} in ${versionFilePath}.`)
  }
