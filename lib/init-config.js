 ...
 * ./wpnv.json, creates ~/wpnv, ~/wpnv/docker-compose.yml, and
 * ~/wpnv/Dockerfile.
 *
...
module.exports = async function initConfig({
  ...
  const configPath = path.resolve('wpnv.json')
  ...
  if (writeChanges) {
    ...
    await writeFile(
      ...
      dockerFileContents(dockerComposeConfig.services.wordpress.image, config)
    )
  } else if (!existsSync(config.workDirectoryPath)) {
    spinner.fail(
      'WordPress environment not yet started. Please run `wpn start` before using any other commands.'
    )
    process.exit(1)
  }
